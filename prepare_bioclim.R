#Download PRISM monthly data and convert to yearly bioclim rasters
...
download_prism=function(){
  ...
  for (month in months){
    for (clim_var in clim_vars){
      get_prism_monthlys(type=clim_var, year = c(testing_years, training_years), mon = month, keepZip=F)
    }
  }
}
...
#####################################################
#Download all the required prism data and create a new
#dataset of bioclimatic rasters for all the configured years. 
download_prism()
